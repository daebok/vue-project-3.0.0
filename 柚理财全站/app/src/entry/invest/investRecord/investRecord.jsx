	    ...
	    let investType = Publiclib.getInvestType(item.investType);
			...
			if(item.rateCouponVal > 0){
				if(useAwardHtml.length>3){
					useAwardHtml += ",加息劵"+item.rateCouponVal+"%";
				}else{
         useAwardHtml  = "使用加息劵"+item.rateCouponVal+"%";
				}
			}
		return (
			<li>
		        <dl>
		          <dt className="dd1">{item.userName}{noviceHtml}</dt>
		          <dd className="dd2">{item.amount}</dd>
		          <dd className="dd3">{createTime}</dd>
		          <dd className="dd4">{investType}</dd>
