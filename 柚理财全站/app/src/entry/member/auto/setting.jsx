  ...
  handleSubmit(e) {
    this.props.form.validateFields((errors, values) => {
      ...
      if( this.state.monthType == "1" ){
        if( monthLimitMin == ""  || monthLimitMax == "" ||  Reg.isNumber(monthLimitMax) ||  Reg.isNumber(monthLimitMin) || (Number(monthLimitMin) > Number(monthLimitMax)) )
          {
            error("请填写正确的月期限");
            return;
          }
      ...
      if( this.state.dayType == "1" ){
        if( dayLimitMin == ""  || dayLimitMax == "" ||  Reg.isNumber(dayLimitMax) ||  Reg.isNumber(dayLimitMin) || (Number(dayLimitMin) > Number(dayLimitMax)) )
          {
            error("请填写正确的天期限");
            return;
          }
