/* 
 * description: mod/inputLimit.js  应用于平台所有输入数字限制
...
function inputLimit(obj,type){
	...
	if(type==7){
		obj.value = obj.value.replace(/(?=[\x21-\x7e]+)[^A-Za-z0-9]/, ''); 
	}
	if(type==8){
		obj.value = obj.value.replace(/^ +| +$/g, '');//不能输入空格
		obj.value = obj.value.replace(/[\u4e00-\u9fa5]/g, '');//不能输入中文
	}
	if(type==9){
		obj.value = obj.value.replace(/[^A-Za-z0-9_@.]/g, '');//只能输入数字、字母、下划线、@、.
	}
}
