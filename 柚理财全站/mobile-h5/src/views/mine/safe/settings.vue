<template>
  <div class="page" id="settings">
    ...
    <mt-cell title="绑定手机" :to="phoneLink" :value="phoneStr" is-link></mt-cell>
    <mt-cell title="绑定邮箱" :to="emailLink" :value="emailStr" is-link></mt-cell>
    <mt-cell title="托管账户" :to="realnameLink" :value="realnameStr" is-link></mt-cell>
    ...
    created(){
      ...
      this.$http.get(ajaxUrl.basicInfo, {params:this.getParams}).then((res) => {
        ...
        if(this.resdata.mobilePhoneStatus == 0){
          this.phoneLink =  '/mine/safe/bind_phone.vue'
          this.phoneStr =  '去绑定'
        }else {
          this.phoneLink =  '/mine/safe/modify_phone?phone=' + this.resdata.mobile
...
//          if(res.resData){
//            this.$toast('上传成功');
//            this.avatar = ajaxUrl.assetsDomain + res.resData.imgUrl
//          }else{
