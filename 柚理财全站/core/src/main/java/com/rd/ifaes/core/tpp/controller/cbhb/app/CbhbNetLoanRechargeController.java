...
@Controller
public class CbhbNetLoanRechargeController extends BaseController{
	/**
	 * 日志记录器
	 */
	private static final Logger LOGGER = LoggerFactory.getLogger(CbhbNetLoanRechargeController.class);
	/**
	 * app充值同步回调
	 * @return
	 */
	@RequestMapping(value = "/cbhbApp/appRecharge/return")
	public String tppReturn(final CbhbNetLoanRechargeModel cbhbNetLoanRechargeModel, final HttpServletRequest request, final Model model) {
		 LOGGER.info("渤海银行APP端app充值进入同步回调");
		 String msg = StringUtils.EMPTY;
		 String resultFlag = StringUtils.EMPTY;
		 cbhbNetLoanRechargeModel.response(this.getRequestMap(request));
		 if(cbhbNetLoanRechargeModel.verifyMac(cbhbNetLoanRechargeModel.getDecryMac())){//验签  默认是OK
	 ...
	 * @throws IOException 
	 */
	@RequestMapping(value = "/cbhbApp/appRecharge/notify")
	public void tppNotify(final CbhbNetLoanRechargeModel cbhbNetLoanRechargeModel,
			final HttpServletRequest request,final HttpServletResponse response) {
		LOGGER.info("渤海银行APP端app充值进入异步回调，回调参数：{}" , getRequestMap(request).toString());
		cbhbNetLoanRechargeModel.response(this.getRequestMap(request));
