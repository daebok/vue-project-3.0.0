...
@Controller
public class JxBatchVoucherPayController extends BaseController {

    private static final Logger LOGGER = LoggerFactory.getLogger(JxBatchVoucherPayController.class);
    @Resource
     ...
     * @date 2017年8月18日
     */
    @RequestMapping(value = "/jxbank/batchVoucherPay/notify")
    public void tppNotify(final String bgData, final HttpServletRequest request, final HttpServletResponse response) {
        LOGGER.info("批次红包发放数据合法性检查的异步回调，回调参数：{}", getRequestParams(request));
        JxBatchVoucherPayModel model = JSON.parseObject(bgData, JxBatchVoucherPayModel.class);
        if (model.responseVerify(model.getDirectResponseValidParamNames())) {
            ...
            if (!JxConfig.SUCCESS.equals(model.getRetCode())) {
                ...
                TppTrade tpp = tppTradeService.findByTradeNoAndType(tradeNo, TppEnum.SERVICE_TYPE_BATCH_VOUCHER.getValue(), TppEnum.STATUS_UNDO.getValue());
     ...
     * @date 2017年8月18日
     */
    @RequestMapping(value = "/jxbank/batchVoucherPay/retNotify")
    public void tppRetNotify(final String bgData, final HttpServletRequest request, final HttpServletResponse response) {
        LOGGER.info("批次红包发放业务处理结果的异步回调，回调参数：{}", getRequestParams(request));
        JxBatchVoucherPayModel model = JSON.parseObject(bgData, JxBatchVoucherPayModel.class);
        if (model.responseVerify()) {
            ...
            if (CacheUtils.incr(handleKey, Constant.DOUBLE_ONE) > Constant.DOUBLE_ONE) {
                LOGGER.info("批次红包发放回调重复处理----orderNo：{}", tradeNo);
                printSuccessReturn(response);
                return;
            }
            CacheUtils.expire(handleKey, ExpireTime.ONE_HOUR);
            TppTrade tpp = tppTradeService.findByTradeNoAndType(tradeNo, TppEnum.SERVICE_TYPE_BATCH_VOUCHER.getValue(), TppEnum.STATUS_UNDO.getValue());
            int count = tppTradeService.countByTradeNoAndType(tradeNo, TppEnum.SERVICE_TYPE_VOUCHER.getValue(), TppEnum.STATUS_UNDO.getValue());
            ...
            if (sucAmount.compareTo(tpp.getMoney()) == 0 && failAmount.compareTo(BigDecimal.ZERO) == 0 && count == sucCounts) {
                //全部成功
                jxbankService.repayVoucherSuccess(tradeNo);
            } else {
                //部分成功
                jxbankService.jxBatchVoucherDetailsHandle(tpp);
            }
        } else {
            LOGGER.error("验签失败：{}", model.getOrderNo());
        }
