...
@Controller
public class JxBatchRepayBailController extends BaseController {

    private static final Logger LOGGER = LoggerFactory.getLogger(JxBatchRepayBailController.class);
    @Resource
     ...
     * @date 2017年8月18日
     */
    @RequestMapping(value = "/jxbank/batchRepayBail/notify")
    public void tppNotify(final String bgData, final HttpServletRequest request, final HttpServletResponse response) {
        LOGGER.info("批次还担保垫付款数据合法性检查的异步通知进入异步回调，回调参数：{}", getRequestParams(request));
        JxBatchRepayBailModel model = JSON.parseObject(bgData, JxBatchRepayBailModel.class);
        if (model.responseVerify(model.getDirectResponseValidParamNames())) {
            ...
            if (!JxConfig.SUCCESS.equals(model.getRetCode())) {
                ...
                TppTrade tpp = tppTradeService.findByTradeNoAndType(tradeNo, TppEnum.SERVICE_TYPE_BATCH_REPAY_BAIL.getValue(), TppEnum.STATUS_UNDO.getValue());
     ...
     * @date 2017年8月18日
     */
    @RequestMapping(value = "/jxbank/batchRepayBail/retNotify")
    public void tppRetNotify(final String bgData, final HttpServletRequest request, final HttpServletResponse response) {
        LOGGER.info("批次还担保垫付款业务处理结果的异步通知进入异步回调，回调参数：{}", getRequestParams(request));
        JxBatchRepayBailModel model = JSON.parseObject(bgData, JxBatchRepayBailModel.class);
        if (model.responseVerify()) {
            String tradeNo = model.getTxDate().concat(model.getBatchNo());
            // 重复处理判断(缓存标记)
            String handleKey = String.format(CacheKeys.PREFIX_REPAY_BAIL_ORDER_NO_HANDLE_NUM.getKey(), tradeNo);
            if (CacheUtils.incr(handleKey, Constant.DOUBLE_ONE) > Constant.DOUBLE_ONE) {
                LOGGER.info("批次还担保垫付款回调重复处理----orderNo：{}", tradeNo);
                printSuccessReturn(response);
                return;
            }
            CacheUtils.expire(handleKey, ExpireTime.ONE_HOUR);
            TppTrade tpp = tppTradeService.findByTradeNoAndType(tradeNo, TppEnum.SERVICE_TYPE_BATCH_REPAY_BAIL.getValue(), TppEnum.STATUS_UNDO.getValue());
            int count = tppTradeService.countByTradeNoAndType(tradeNo, TppEnum.SERVICE_TYPE_REPAY_BAIL.getValue(), TppEnum.STATUS_UNDO.getValue());
            ...
            if (sucAmount.compareTo(tpp.getMoney()) == 0 && failAmount.compareTo(BigDecimal.ZERO) == 0 && count == sucCounts) {
                //全部成功
                jxbankService.repayBailSuccess(tpp);
            } else {
                //部分成功
