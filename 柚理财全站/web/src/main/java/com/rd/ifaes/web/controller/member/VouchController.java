...
public class VouchController extends WebController {
    ...
    public Object verifyList(final Borrow model) {
        ...
        borrowList = borrowService.findBasePage(model);
        //处理期限真实姓名
    ...
    public Object overdueList(final ProjectRepayment model, final String overdueStatus) {
        ...
        if (RepaymentEnum.STATUS_OVERDUE_ADVANCE_NO.eq(overdueStatus)) { //未垫付
            model.setStatus(RepaymentEnum.STATUS_NOT_REPAY.getValue());
        } else {
            model.setStatus(RepaymentEnum.STATUS_REPAID.getValue());
            model.setRepayTypeAdvance(RepaymentEnum.STATUS_OVERDUE_ADVANCE_YES.getValue());
        }
        data = new HashMap<>();
        data.put(RESULT_NAME, true);
    ...
    public String overduePay(final ProjectRepayment projectRepayment, final Model model, final HttpServletRequest request) {
    ...
    @ResponseBody
    public Object offlineRepay(final String uuid) {
        projectRepaymentService.offlineRepay(uuid);
        return renderSuccessResult();
    }
    ...
    @ResponseBody
    public Object getAdvanceRepayCode(String repaymentId) {
     ...
     * @return
     */
    @RequestMapping(value = "/member/vouch/doRepay")
    @ResponseBody
    @TokenValid(value = TokenConstants.TOKEN_REPAY, dispatch = true)
    public Object doRepay(final ProjectRepayment projectRepayment, final String code) {
        final User currentUser = getSessionUser();
        projectRepaymentService.repay(projectRepayment.getUuid(), currentUser.getMobile(), code, currentUser.getUuid());
        return renderSuccessResult();
    }
