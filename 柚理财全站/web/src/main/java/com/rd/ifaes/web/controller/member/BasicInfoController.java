...
public class BasicInfoController extends WebController {
	...
	public Object getIdentifyInfo(){
		...
		//头像路径
		final UserCache userCache = userCacheService.findByUserId(user.getUuid());
		data.put("avatar_photo", userCache.getAvatarPhoto());
		//安全等级
		data.put("securityLevel", identifyService.getSecurityLevel(user));
		data.put("securityValue", identifyService.getSecurityValue(user));
		//上次登录时间
		final Date lastLoginTime = loginLogService.getLastLoginTime(user.getUuid());
		data.put("lastLoginTime", lastLoginTime);
		//托管账户
		data.put("tppUserCustId", user.getHideTppUserCustId());
		//手机号码
		...
		data.put("isVouch", userCache.getUserNature());
		if(!UserCache.USER_NATURE_PERSON.equals(userCache.getUserNature())){
		...
		data.put("tppName", TppServiceEnum.nameOf(ConfigUtils.getTppName()).getDesc());
		

		data.put("payPwd", user.getPayPwd() != null ? "1" : "0");
		return data;
	}
	...
	@ResponseBody
	public Object modifyUserInfo(final HttpServletRequest request,final UserBaseInfo userBaseInfo,
