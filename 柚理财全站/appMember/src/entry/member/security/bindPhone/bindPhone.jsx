   ...
   handleSubmit(e) {
    ...
    this.props.form.validateFields((errors, values) => {
      ...
      .done(function(data) {
        if(data.result){
           $("#j-select").remove();
           ReactDOM.render(<Step_second reqType={$("#reqType").val()} srvTxCode={$("#srvTxCode").val()} checkType = {self.props.checkType} modifyPhoneSign={data.modifyPhoneSign}/>,  document.getElementById("j-step-second"));
           $(".showbox").hide();
           $("#j-step-second").show();      
        }
        else
...
let Email = React.createClass({
  ...
  render() {
    const { getFieldProps, getFieldError, isFieldValidating } = this.props.form;
    const idNoProps = getFieldProps('idNo',{
        validate: [
        {
          rules:[
        ...
        <div className="row" style={{width:400}}>
          ...
          <div className="col-18 pl20">
            <FormItem hasFeedback>
              <Input {...idNoProps} type="text" className="input long-input" maxLength="18" autoComplete="off" placeholder="请输入身份证号码"  />
            </FormItem>
          </div>
        </div>      
        <div className="row" style={{width:400}}>
  ...
  getcode(e){
    ...
    else
    {
      $.ajax({
        ...
        type: 'POST',
        dataType: 'json',
        data: {mobile:mobile,reqType:this.props.reqType,srvTxCode:this.props.srvTxCode},
      })
      .done(function(data){
          if(data.result == false)
          {
...
let BindPhone = React.createClass({
  ...
  selectHtml(data){
    ...
    let list = [<Option key="1" value="01">通过手机验证</Option>,<Option key="2" value="02">通过邮箱+身份证</Option>,<Option key="3" value="04">通过身份证+密保问题</Option>],typelist = [],listvalue=[];
    if(data.mobile){
      typelist.push(list[0]);
      listvalue.push("01");
    }
    if(data.email && data.realNameStatus == "1"){
      typelist.push(list[1]);
      listvalue.push("03");
    }
    if(data.questionList.length && data.realNameStatus == "1"){
      typelist.push(list[2]);
      listvalue.push("04");      
    }
    this.setState({
  ...
  render(){
    ...
    const { getFieldProps, getFieldError, isFieldValidating } = this.props.form;
    const componentArr = [<Phone checkType={this.state.selecTvalue} mobile = {data.mobile} />, <Email checkType={this.state.selecTvalue} email = {data.email} />, <PwdQuestion checkType={this.state.selecTvalue} questionList={data.questionList} />];
    const html_1 = componentArr[0],html_2 = componentArr[1],html_3 = componentArr[2];
    ...
    return (
      <div className="account-main">                	
      	...
      	<div className="bindPhone form-themes">
            ...
            <div className="showbox" id="form_02" >{html_2}</div>
            <div className="showbox" id="form_04" >{html_3}</div>
