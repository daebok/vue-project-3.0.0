...
let CompanyAllInfo = React.createClass({
    ...
    isNumber(rule, value, callback) {
        if(!value){
            callback();
        }else if(Reg.isNumber(value)||value.length!=15){
            callback([new Error('请输入15位营业执照注册号')]);
        }else{
            $.ajax({
                url: '/member/security/checkRegCode.html',
                type: 'POST',
                dataType: 'json',
                data: {
                    bussinessCode:value
    ...
    regexChnName(rule, value, callback) {
        if(!value){
            ...
            if(!Reg.regexChnName(value)||value.length<2){
                callback([new Error('请输入2-10个字符的真实姓名')]);
            }else{
                callback();
            }
        }
    },
    zzjg(rule, value, callback) {
        if(!value){
            callback();
        }else{
            if(value.length<9){
                callback([new Error('请输入组织机构代码')]);
            }else if(!/^[0-9A-Z]{9}$/.test(value)){
                callback([new Error('请输入正确的组织机构代码，字母必须为大写')]);
            }else{
                $.ajax({
                    url: '/member/security/checkRegCode.html',
                    type: 'POST',
    ...
    render(){
        ...
        const bussinessCodeProps = getFieldProps('bussinessCode', {
            validate: [{
                rules: [
                    {required: true, message: '请输入15位营业执照注册号'},
        ...
        const orgCodeProps = getFieldProps('orgCode', {
            validate: [{
                rules: [
                    {required: true, message: '请输入组织机构代码，格式XXXXXXXXX'},
                    {validator: this.zzjg},
                ], trigger: 'onBlur'
            }]
        });
        ...
        const legalPersonProps = getFieldProps('legalPerson', {
            rules: [
                {required: true, message: '请输入您的真实姓名，仅为中文'},
                {validator: this.regexChnName}
            ],
        });
        ...
        return (
            <div>
                <Form horizontal className="form-themes" target="_blank"
                    ...
                    <input name="threeCertificate" value="0" type="hidden"/>
...
let CompanyMinInfo = React.createClass({
    ...
    xydm(rule, value, callback) {
        if(!value){
            ...
            if(value.length<18){
                ...
                callback([new Error('仅能输入数字或大写英文字母')]);
            }else{
                $.ajax({
                    url: '/member/security/checkRegCode.html',
                    type: 'POST',
    ...
    render(){
        ...
        const creditCode = getFieldProps('creditCode', {
            validate: [{
                rules: [
                    {required: true, message: '请填写18位统一社会信用代码'},
                    {validator: this.xydm}
                ], trigger: 'onBlur'
            }]
        });
        ...
        return (
            <div>
                <Form horizontal className="form-themes" target="_blank"
                    ...
                    <input name="threeCertificate" value="1" type="hidden"/>
                    <Input {...returnUrlProps} name="returnUrl" type="hidden"/>
                    <div className="row" style={{width: 400}}>
                        <div className="col-7">&nbsp;</div>
                        <div className="col-17 pl20">
...
let PayaccountNo = React.createClass({
  getInitialState() {
    return {
    };
  },
  ...
  render(){
            ...
            return (
                <div className="account-main">
                    <div className="main-title">开通托管账户</div>
                    <p className="remind" style={{"marginTop":"100px","fontSize":"18px"}}><em className="iconfont">&#xe639;</em>请联系客服人员开通托管账户</p>
                </div>
              )
          }  
});
...
let Regok = React.createClass({
  ...
  render(){
    ...
    const content = [<dl><dt><span className="success">已经绑定江西银行</span><s>江西银行账户：</s>{this.props.data.tppUserCustId}</dt><dd><a href={this.props.data.tpp_login_url} target="_blank">进入江西银行</a></dd></dl>,<dl><dt><span className="success">企业开户审核不通过</span><s>{data.auditDesc}</s></dt><dd><a href="javascript:" onClick={this.reg}>重新开户</a></dd></dl>,<dl><dt><span className="success">企业开户审核中</span><s>开户申请时间：</s>{Publiclib.formatDate(data.realNameVerifyTime,2)}</dt></dl>][data.realNameStatus-1];    
...
function getinfo(){
    ...
    .done(function(data){
      ...
      else
      {
        ...
        {
          ReactDOM.render(<PayaccountNo />,  document.getElementById("j-paydetailmain"));     
        } 
