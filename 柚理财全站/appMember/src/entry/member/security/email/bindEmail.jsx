...
let BindEmail = React.createClass({
    ...
    this.props.form.validateFields((errors, values) => {
      ...
      .done(function(data) {
        ...
        else
        {
          error(data.msg);
          self.callback();
          self.props.form.resetFields(["code"]);
        }
      })
      .fail(function() {
        self.callback();
  ...
  getcode(e){
    ...
    .done(function(data) {
      if(data.result){
        ReactDOM.render(<CountDown msg={"邮件已经发送到您的邮箱"+ self.refs.email.props.value +"，请注意查收！"} time = '60' text='秒后重新获取' callback = {self.callback}/>, document.getElementById("countdown"));
      } else {
