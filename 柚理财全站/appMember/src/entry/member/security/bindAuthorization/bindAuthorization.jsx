...
let AuthSign = React.createClass({
    getInitialState() {
      return {
        timeflag:false,
        visible:false
  ...
  render() {
    ...
    return (
      <div className="account-main">
        <div className="main-title">设置业务授权</div>
        <Form horizontal className="form-themes" target="_blank" method="post" action="/member/security/authSign.html" form={this.props.form} onSubmit={this.handleSubmit}>
        ...
        <Modal className="orderPayModal" ref="modal"
            ...
            title="业务授权" onOk={this.handleOk} onCancel={this.handleCancel}
            width={480}
            footer={[
              <Button key="submit" type="primary" size="large" onClick={this.handleOk}>
                设置成功
              </Button>,
              <Button key="back" type="ghost" size="large" onClick={this.handleCancel}>设置失败</Button>,
            ]}
          >
          <div className="modal-content"><i className="iconfont">&#xe62e;</i><p>请在新打开的页面中完成业务授权</p></div>
          </Modal>
      </div>
    );
  }
...
AuthSign = createForm()(AuthSign);
ReactDOM.render(<AuthSign mobile={$("#mobile").val()} reqType={$("#reqType").val()} srvTxCode={$("#srvTxCode").val()} />,  document.getElementById("j-bindPhonemain"));
