  ...
  Modal.success({
    ...
    this.props.form.setFieldsValue({
      ...
      //legalDelegate:data.userCompanyInfo.legalDelegate,
    	...
    	companyIndustry:data.userCompanyInfo.companyIndustry,
    	businessScope:data.userCompanyInfo.businessScope,    	
    	companyScale:data.userCompanyInfo.companyScale,
    	repaySource:data.userCompanyInfo.repaySource,
    });
    ...
    //     initialValue: self.state.dateInitialValue ? self.state.dateInitialValue : data.userCompanyInfo.establishDate ? Publiclib.formatDate(data.userCompanyInfo.establishDate) : ,
      ...
      //   });      
	    ...
	    const companyIndustryProps = getFieldProps('companyIndustry', {
	    	rules: [{ required: true, message: '请填写公司行业' }],
	    });
	    const repaySourceProps = getFieldProps('repaySource', {
	    	rules: [{ required: true, message: '请填写还款来源' }],
	    });
	    const companyScaleProps = getFieldProps('companyScale', {
	    	rules: [{ required: true, message: '请填写公司规模' }],
	    });
	    const businessScopeProps = getFieldProps('businessScope', {
	    	rules: [{ required: true, message: '请填写主营业务' }],
	    });


	    //判断是否已修改过一次
	   	let userNameHtml = <FormItem hasFeedback><Input {...userNameProps} type="text" className="input long-input" maxLength="20" autoComplete="off" placeholder="6-20位字符，由字母或字母加数字组成" /></FormItem>;
	   	if(data.userCache.renameFlag === "1"){
	   	  	userNameHtml = <FormItem><p className="txt">{data.userName}</p></FormItem>;
	   	}			
	    return (
	    	<Form horizontal form={this.props.form} onSubmit={this.handleSubmit}>               
	    		<ul className="form-list form-list-edit">
					...
					<li>
						<label className="label">公司行业</label>
						<div className="form-item">
							<FormItem hasFeedback>
								<Input {...companyIndustryProps} type="text" className="input long-input" maxLength="30" autoComplete="off" placeholder="请输入公司行业"  />
	              			</FormItem>
						</div>
					</li>
					<li>
						<label className="label">还款来源</label>
						<div className="form-item">
							<FormItem hasFeedback>
								<Input {...repaySourceProps} type="text" className="input long-input" maxLength="30" autoComplete="off" placeholder="请输入还款来源"  />
	              			</FormItem>
						</div>
					</li>
					<li>
						...
						<div className="form-item">
							<FormItem hasFeedback>
								<Input {...companyScaleProps} type="text" className="input long-input" maxLength="30" autoComplete="off" placeholder="请输入公司规模"  />
	              			</FormItem>
						</div>
					</li>
					<li>
						<label className="label">主营业务</label>
						<div className="form-item">
							<FormItem hasFeedback>
								<Input {...businessScopeProps} type="textarea" className="input long-input" maxLength="30" autoComplete="off" placeholder="请输入主营业务"  />
	              			</FormItem>
						</div>
					</li>
					<li>
						...
						<div className="form-item">
							...
							<div className="line-inputBox">
								<FormItem hasFeedback>
									<Input {...officeAddress} type="text" className="input long-input" maxLength="50" style={{width:'400px'}} autoComplete="off" placeholder="具体地址，不超过50个字符"  />
								</FormItem>
							</div>
						</div>
					</li>
