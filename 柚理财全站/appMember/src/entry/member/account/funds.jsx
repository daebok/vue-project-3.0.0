...
const columns = [{
            ...
            render(text,record) {
              ...
              let newText = text ? text.substr(0,37) : "-";
              let html=<span>{newText}</span>
              if(record.htmlAUrl!=''){
                html=<a href={record.htmlAUrl} className="remark-link"><span>{newText}</span></a>
              }
              if(text && text.length > 37){
