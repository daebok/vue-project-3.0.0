...
let Mybondsetting = React.createClass({
  ...
  compute(e){
    ...
    let bondMoney = $("#bondMoney").val(), bondApr = Number(this.bondApr);
    if( bondMoney != "" && (bondApr === 0 || bondApr != "") && !isNaN(bondMoney) && !isNaN(bondApr) ){
  ...
  checkNumber(rule, value, callback) {
    ...
    getInfo(){
        ...
        $.ajax({
            ...
            type: 'POST',
            dataType: 'json',
            data: {protocolId:self.props.protocolId},
        })
            .done(function(data) {
                if(data.result){
                    self.setState({
    ...
    componentDidMount(){
        ...
        this.props.form.setFieldsValue({
            bondApr:apr.toString()
        });
        let bondMoney = $("#bondMoney").val(), bondApr = Number(apr);
        if( bondMoney != "" && (bondApr === 0 || bondApr != "") && !isNaN(bondMoney) && !isNaN(bondApr) ){
            $("#j-computeResult").html(((1 + (bondApr/100))*bondMoney).toFixed(2));
        }
    },
          ...
          <Form horizontal className="form-themes" style={{marginLeft:100}} form={this.props.form} onSubmit={this.handleSubmit}>
              <div className="row" style={{width:400}}>
                ...
                <div className="col-18 pl20 form-text lab">
                   <span className="form-text"><span className="form-textsm">{this.props.bondMoney}</span>元{this.props.repayedCapital > 0 ? "(不包含已还款本金"+ this.props.repayedCapital +"元)" : ""}</span>
                </div>
              </div>
              <div className="row" style={{width:400}}>
              ...
              <div className="row" style={{width:400}}>
                ...
                <div className="col-18 pl20 hasunit hasunit-number">
                  <FormItem hasFeedback  extra={maxtext}><InputNumber placeholder="请输入折溢价率" {...bondAprProps} size="large"  step={0.1} min={Number(this.props.min)} max={Number(this.props.max)} onChange={this.computeChange} onKeyUp={this.filter} /></FormItem>
              ...
              <div className="row" style={{width:400}}>
                ...
                <div className="col-18 pl20">
                  <FormItem validateStatus="error" {...agreeProps}  help = {this.state.label} ><Checkbox ref="check" checked={this.state.checked} onChange={this.onChange} ></Checkbox>我已阅读并同意<a href="javascript:;" onClick={this.getInfo}>《债权转让协议》</a></FormItem>
                </div>
              </div>
              <Modal
              ...
              >
                  <div style={{maxHeight:"400px",overflowY:"auto",overflowX:'hidden'}} dangerouslySetInnerHTML={{__html: this.state.content}}></div>
              </Modal>
