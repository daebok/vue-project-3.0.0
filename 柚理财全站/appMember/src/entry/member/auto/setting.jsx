...
import { Button,Switch,Form,Input,Modal,Checkbox,Radio} from 'antd';
...
class Auto extends React.Component{
  ...
  handleSubmit(e) {
    this.props.form.validateFields((errors, values) => {
      ...
      if( this.state.monthType != "1" && this.state.dayType != "1"){
        error("请至少选择一种投资期限");
        return;
      }
  ...
  getInfo(){
      ...
      .done(function(data) {
        ...
        if(typeof data.result != undefined && data.result){
           ...
           if(OptionsId[0] == 0)
           {
            ...
            self.setState({
              ...
              valueRed:data.rule.redType,
              valueInc:data.rule.increaseType,         
           });
        ...
        {
          redirect(data.msg,data.url,data.buttonName);
        }  
      })
      .fail(function() {
        error('网络异常，请重试！');
  ...
  setdefault(data){
    if(data.rule.amountDayMax){
      this.props.form.setFieldsValue({
        amountDayMax:data.rule.amountDayMax.toString()
      });
    }
    if(data.rule.aprMin){
      this.props.form.setFieldsValue({
        aprMin:data.rule.aprMin.toString()
      });
    }
  ...
  CheckboxChange(e){
    if(e.target.id == "j-monthType"){
      this.setState({
        monthType:e.target.checked ? "1" : "",
      });
    }
    else
    {
      this.setState({
        dayType:e.target.checked ? "1" : "",
      });      
    }  
  }
  canuse(e){   
  ...
  render(){
        ...
        const redTypeProps = getFieldProps("redType",{
            initialValue:this.state.valueRed
        });
        const increaseTypeProps = getFieldProps("increaseType",{
            initialValue:this.state.valueInc
        });
          
    return(
      <div>
        <div className="auto-nav">
          <div className="auto-control"><spn className="control-status">{this.state.display ? "已经开启" : "已经关闭"}</spn><Switch checked={this.state.display} onChange={this.switch} /></div>
             ...
             <Form horizontal className="form-themes" form={this.props.form} onSubmit={this.handleSubmit}>
                ...
                <div className="row" style={{width:800,height:"auto",overflow:"hidden"}}>
                  <div style={{overflow:"hidden"}}>
                    ...
                    <div className="col-2">
                      <FormItem ><Input className="input-sm" {...monthLimitMinProps} maxLength="2" /></FormItem>
                    </div>
                    <div className="col-1 lab lab-line">-</div>
                    <div className="col-2">
                      <FormItem ><Input className="input-sm" {...monthLimitMaxProps} maxLength="2" /></FormItem>
                    </div>
                ...
                <div className="row" style={{width:800}}>
                  <div className="col-3 lab form-laber textR">选择红包</div>
                  <div className="col-10 pl20 hasunit" style={{paddingTop:12}}>
                    <RadioGroup onChange={this.onChangeRed} value={this.state.valueRed}>
                      <Radio key="不使用" value={0}>不使用</Radio>
                      <Radio key="最早到期日" value={1}>最早到期日</Radio>
                      <Radio key="最大金额" value={2}>最大金额</Radio>
                    </RadioGroup>
                  </div>
                </div>
                <div className="row" style={{width:800}}>
                  <div className="col-3 lab form-laber textR">选择加息</div>
                  <div className="col-10 pl20 hasunit" style={{paddingTop:12}}>
                    <RadioGroup onChange={this.onChangeInc} value={this.state.valueInc}>
                      <Radio key="不使用" value={0}>不使用</Radio>
                      <Radio key="最早到期日" value={1}>最早到期日</Radio>
                      <Radio key="最大金额" value={2}>最大比例</Radio>
                    </RadioGroup>
                  </div>
                </div>
                <div className="row" style={{width:800,position:"relative",top:-17,height:49,overflow:"hidden"}}>
                  ...
                  {/*<div className="col-5 pl20">
                    <FormItem className="fl"><Checkbox id="j-realizeUseful" checked={this.state.realizeUseful == "1" ? true : false} onChange={this.canuse}></Checkbox></FormItem><span className="fl checkboxtext"  >仅可变现产品</span>
                  </div>*/}
                  <div className="col-5 pl20">
                    <FormItem className="fl"><Checkbox id="j-bondUseful" checked={this.state.bondUseful == "1" ? true : false} onChange={this.canuse} ></Checkbox></FormItem><span className="fl checkboxtext"  >仅可转让产品</span>
                  </div>                  
                </div>                                                                  
                <Input {...monthTypeProps}  type="hidden" />                      
                <Input {...dayTypeProps}  type="hidden" />
                <Input {...redTypeProps}  type="hidden" />
                <Input {...increaseTypeProps}  type="hidden" />                     
                <div className="row" style={{width:800}}>
