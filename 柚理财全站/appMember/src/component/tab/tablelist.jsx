...
export default class Tabs extends React.Component{
  ...
  setstatus(e){
    this.refs.clean.clean();
    if(e.target.nodeName == "SPAN"){
      $("#"+ this.props.tableid + "filter .filter-fixedtimes span").removeClass('on');
      $(e.target).addClass('on');
      this.datastatus = e.target.getAttribute("data-status");
      this.getlist(this.props.url,'','',this.state.selectvalue);
    }
  }
  ...
  getlist(urls,stime,etime,selectvalue,page){
    ...
    let datas = {dateType:that.datatype,optType:that.datastatus,startTime:stime,endTime:etime,"page.page":pages,"page.pageSize":that.props.pageSize ? that.props.pageSize : "10"};
  ...
  render() {
    ...
    let filterclassname ="row filter",filterSelectClassname ="filter-select",filterSectiontime="filter-sectiontime",filterFixedtime="filter-fixedtime",filterFixedtimes="filter-fixedtimes";
    if(this.props.config){
      if(!this.props.config.single && !this.props.config.record &&!this.props.config.datetime && !this.props.config.select)
      {
        ...
        if(!this.props.config.record)
        {
            filterFixedtimes = "filter-fixedtimes hide";
        }
    ...
    let singleArr =<div className="Sselect"><em>日期范围</em><ul className="clearfix" onClick={this.set}>
    ...
    let recordArr =<div className="Sselect"><em>记录类型</em><ul className="clearfix" onClick={this.setstatus}>
                  <li><span className="on" data-status="">全部</span></li>
                  <li><span data-status="1">获取记录</span></li>
                  <li><span data-status="0">使用记录</span></li>
                </ul></div>;
    ...
    if(this.props.recordArr){
      ...
      {
        let self = this;
        liList = this.props.recordArr.map(function(el, index) {
          let on = "";
          if(index == 0){
            on = "on";
          }
          return <li key={index+1}><span className={on} data-status={self.props.recordArrId[index]}>{el}</span></li>;
        });

        recordArr = <ul className="clearfix" onClick={this.setstatus}>{liList}</ul>;
      ...
      else
      {
        ...
        liList = this.props.recordArr.map(function(el, index) {
          return <li key={index+1}><span data-status={index}>{el}</span></li>;
        });

        recordArr = <ul className="clearfix" onClick={this.setstatus}><li  key="0"><span className="on" data-status="">全部</span></li>{liList}</ul>;
    ...
    return (
          <div className="tablelist">
            <div className={filterclassname} id={filterid}>
              ...
              <div className={filterFixedtimes}>
                {recordArr}
              </div>
