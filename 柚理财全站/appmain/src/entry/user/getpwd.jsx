...
var phoneCodeFlag = false,emailCodeFlag = false;
    ...
    .done(function(data) {
      if(data.result){
        ReactDOM.render(<CountDown msg={"验证码已经发送到您的手机，请注意查收！"} time = '60' text='秒后重新获取' callback = {self.callback}/>, document.getElementById("countdown2"));
          ...
          $(".codeimg")[0].click();
          self.props.form.resetFields(['imgValidCode']);
...
checkMobile(rule, value, callback) {//手机号是否存在
    ...
    //   }
    //   $.ajax({
    //     url: '/user/security/checkPathWay.html',
    //     type: 'POST',
    //     dataType: 'json',
    //     data:{pathWay:value,type:"1"}
    //   })
    //   .done(function(data) {
    //     if(data.result){
    //       that.setState({
  ...
  render() {
    ...
    const validCodeProps = getFieldProps('imgValidCode', {
      validate: [
        {
          rules: [
            { required: true, whitespace: true, message: '请输入图形验证码' }
          ],
         trigger: ['onBlur'],
        },
        // {
      ...
      <div>
        ...
        <div className="row" style={{width:400}}>
          ...
          <div className="col-9 pl20">
            <FormItem><Input {...validCodeProps} type="text" maxLength="4" autoComplete="off" placeholder="输入验证码"  />
            </FormItem>
          </div>
          <div className="col-9 pl20">
...
let ModifyPwd = React.createClass({
  ...
  handleSubmit(e) {
    ...
    this.props.form.validateFields((errors, values) => {
      ...
      console.log(values);
      let self = this;
      if(this.state.type == 1 && !phoneCodeFlag){
        error("请点击获取短信验证码");
        return;
      }
      if(this.state.type == 2 && !emailCodeFlag){
        error("请点击获取邮箱验证码");
        return;
      }
