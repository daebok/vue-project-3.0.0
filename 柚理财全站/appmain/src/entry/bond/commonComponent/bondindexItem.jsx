...
class BondStatus extends React.Component{	
	render(){
		...
		if(item.borrowStatus != "9"){//没有全部回款 回款中
			if(item.status == "3"){//债权转让完成
				statusHtml = <div className="bondment exhausted">共{item.totalPeriod}期 已还{item.repayedPeriod}期
				</div>
			} else if(item.status == "0"){//立即投资
				if(item.remainMoney=="0"){
					statusHtml = <div className="bondment exhausted">已售罄</div>
				}else{
...
export default class BondindexItem extends React.Component{
	render(){
		let item = this.props.item;		
	    //详情页面链接
		...
		return (
		        ...
		        <dl>
		          ...
		          <dd className="timelimit"><span className="f20">{item.remainDays}</span>天</dd>
		          <dd className="minmoney"><span className="f20">{item.bondLowestMoney}</span><span>元</span></dd>
		          <MoneyStatus item={item}/>
		          <BondStatus item={item}/>
		        </dl>
		   </li>
		)
	}
