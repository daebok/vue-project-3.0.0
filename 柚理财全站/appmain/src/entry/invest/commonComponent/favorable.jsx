...
let AddInterestSum = 0, sum = 0,ids = [];
...
class RedPacket extends React.Component{
    ...
    selectRedPacket(){
        ...
        let self = this;
        let flag = true;
        $("body").on("click", ".redPacketList li:not(.noText)", function(){                     
            ...
            if(icon.css("display") == "none" && flag && (sum+value) <= self.objVal.moneyYes){               
                ...
                //console.log(typeof(sum),typeof(value))
                sum = Number(sum.toFixed(2)) + Number(value.toFixed(2));
                //console.log(parseFloat(sum.toFixed(2)))
                //console.log(parseFloat(sum.toFixed(2)),parseInt(sum.toFixed(2)))
            ...
            //设置选择红包总额
            self.props.selectFn(parseFloat(sum.toFixed(2)), ids);                   
        });
    }
    removeIds(ids, id){
        let len = ids.length;
...
class AddInterest extends React.Component{
    ...
    selectAddInterest(){        
        let self = this;
        $("body").on("click", ".addInterestList li:not(.noText)", function(){               
            let icon = $(this).find(".favorable-icon");
            let value = parseFloat($(this).find(".favorable-value").attr("data-val"));
            let id = "";
            ...
            // }            
            //设置选择加息券
            self.props.selectFn(AddInterestSum, id);                   
        });
    }
    render(){
        let data = this.props.data;
...
export default class Favorable extends React.Component{
    ...
    reset(){
       ...
       sum = 0;
       ids = [];
       $("#favorableText").removeAttr('data-red');
    }
    ...
    handleOk() {
        ...
        //计算实际支付
        this.props.setRealPayAmount(parseFloat(this.amountVal - this.state.selectRedPacketRental),this.state.selectAddInteres);            
    } 
    handleCancel() {
    ...
    getRedPacketData(id, amountVal,current){ 
        ...
        $.ajax({
            ...
            _this.setDisplay("input[name='redEnvelopeIds']",".redPacketList li");
        }).fail(function() {
            error('网络异常，请重试！');
        });
    }
    setDisplay(input,dom){
        $(dom).find(".favorable-icon").hide();
        let ids = $(input).val().split(",");
        $.each($(dom),function(k, v) {
           $.each(ids,function(index, el) {       
              if($(v).attr("data-id") === el){
                 $(v).find(".favorable-icon").show();
              }
           });
        });
    }
    getAddInterestData(id, amountVal,current){
        ...
        $.ajax({
            ...
            data: {"projectId": id, "tenderMoney": amountVal, "page.pageSize": 6,"page.page":current}
             ...
             _this.setDisplay("input[name='rateCouponId']",".addInterestList li");
        }).fail(function() {
            error('网络异常，请重试！');
        });
    }
