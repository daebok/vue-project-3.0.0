...
class InvestStatus extends React.Component{
	...
	render(){
		...
		switch (item.status)
		{
			...
			case "49":
				if(flag && item.bespeak == "0"){
					...
					if(this.props.item.remainAccount=='0'){
						html = <div className="investment exhausted">已售罄
						</div>;
					}else{
					html = <a href={link} className="investment" target="_blank" >立即投资</a>
					}
				}													
				break;
			case "5":
				html = <div className="investment exhausted">已售罄</div>;
				break;
			case "6":
			case "8":
			case "87":
			case "88":
			case "90":
				html = <div className="investment exhausted">共{item.periods}期 已还{item.repayedPeriods}期
				</div>;
				break;			
			case "9":
			case "91":
...
export default class InvestindexItem extends React.Component{
    ...
    let realizeFlag = item.realizeUseful == "1" ?	<span className="tags realize">可变现</span> : "";
		...
		return (
				<li>
		        <dl>
		          <dt><a href={link} target="_blank" title={item.projectTypeName + item.projectNo}>{item.projectTypeName}-{item.projectNo}</a>{noviceHtml}{transferHtml}{realizeFlag}</dt>
		          <dd className="profit">{item.apr}<em>%</em>{addAprHtml}</dd>
		          <dd className="timelimit"><span className="f20">{item.timeLimit}</span>{item.timeType == "0" ? "个月" : "天"}</dd>
		          <dd className="minmoney"><span className="f20">{item.lowestAccount}</span><span>元</span></dd>		          
