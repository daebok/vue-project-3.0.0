...
export default class OrderCopies extends React.Component{
  constructor(props){
    ...
    this._handleCancel2 = this.handleCancel2.bind(this);
  ...
  getRedRateFlag(){
    ...
    $.when(Rate,Red).done(function(a1,a2){
      if( (a1[0].data.rows==undefined) && (a2[0].data.rows==undefined) ){
        self.setState({
          showRedRateTip : false
        });
      }
      else if((a1[0].data.rows==undefined) && ($("#additionalRateUseful").val()==1) && ($("#redEnvelopeUseful").val()!=1)){
        self.setState({
          showRedRateTip : false
        });
      }else if((a2[0].data.rows==undefined) && ($("#redEnvelopeUseful").val()==1) && ($("#additionalRateUseful").val()!=1)){
        self.setState({
          showRedRateTip : false
        });
      }else{
  ...
  countIncomeHander(e){
    ...
    if(el.value==""){
      ...
      this.refs.amount.value=parseInt(el.value);
    }
  ...
  setRealPayAmount(val,interests){
    this.RxgAccountYesSecond(val,this.ErrorShow)
    if(interests >= 0){
  ...
  RxgAccountYesSecond(value, callback){
    value = parseFloat($("#amount1").val());
      ...
      console.log(value,copyAccount,remAccount,min,userMoney,this.objVal.lowestAccount)
      if(!value || value == "0"){
          callback("请输入份数");
      }else if(min==0){
      ...
      <div className="order-main">
        <Form horizontal target="_blank" method="post" action={formAction} form={this.props.form} onSubmit={this._handleSubmit}>
              ...
              <dd>
                  <div className="form-input">
                      <div className={this.state.stateClass}>
                          <input type="text" name="amount1" autoComplete="off" id="amount1" ref="amount" className="ant-input ant-input-lg" maxLength="10" placeholder='请输入份数' onKeyUp={this.countIncomeHander}/>
          ...
          <Modal 
            ...
            footer={[]}
          >
            <Verification form={this.props.form} ref="vTip" />
          </Modal>
