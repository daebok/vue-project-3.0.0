...
public class CacheUtils {
	...
	//redis key 前缀
	public static String REDIS_KEY_PREFIX = PropertiesUtils.getValue("redis.keyPrefix");
	...
	public static <T> T getJson(String key, Class<T> clazz) {
		return JsonMapper.fromJsonString(stringRedisTemplate.boundValueOps(appendKeyPrefix(key)).get(), clazz);
	}
	...
	public static void set(String key, Object value, ExpireTime time) {
		...
		if (null == key || null == value) {
			throw new BussinessException("key or value must not null");
		}
	...
	public static void setJson(String key, Object value, ExpireTime time) {
		stringRedisTemplate.opsForValue().set(appendKeyPrefix(key), JsonMapper.toJsonString(value));
		if (time.getTime() > 0) {
			redisTemplate.expire(appendKeyPrefix(key), time.getTime(), TimeUnit.SECONDS);
		}
	}
	...
	public static void setDouble(String key, double value, ExpireTime time) {
		...
		if (time.getTime() > 0) {
			stringRedisTemplate.expire(appendKeyPrefix(key), time.getTime(), TimeUnit.SECONDS);
		}
	}
	...
	public static <T> T getMap(String key, Class<T> clazz) {
		BoundHashOperations<String, String, String> boundHashOperations = redisTemplate.boundHashOps(appendKeyPrefix(key));
	...
	public static boolean setnx(String key, String value, ExpireTime cacheTime) {
		...
		if (cacheTime.getTime() > 0) {
			redisTemplate.expire(appendKeyPrefix(key), cacheTime.getTime(), TimeUnit.SECONDS);
		}
		return flag;
	}
	...
	public static void setIntForPhone(String key, Object value, int time) {
		stringRedisTemplate.opsForValue().set(appendKeyPrefix(key), JsonMapper.toJsonString(value));
		if (time > 0) {
			stringRedisTemplate.expire(appendKeyPrefix(key), time, TimeUnit.SECONDS);
		}
	}
	 ...
	 * @return
	 */
	public static String appendKeyPrefix(String key){
		return StringUtils.isNull(REDIS_KEY_PREFIX).concat(":").concat(key);
	}
