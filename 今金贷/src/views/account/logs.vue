<template>
  <div class="page" id="record">
    <mt-header class="bar-nav" title="资金明细" >
      <mt-button slot="left" icon="back" v-back-link></mt-button>
    </mt-header>
    <div class="page-loadmore-wrapper" ref="wrapper" :style="{ height: wrapperHeight + 'px' }">
      <!-- <mt-loadmore :top-method="newList.length > 6 ? loadTop : ''" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" ref="loadmore"> -->
      <mt-loadmore :top-method="loadTop" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" ref="loadmore">
        <ul class="record-list">
          <div v-for="(item, index) in newList">
            <div class="record-date">{{item.timeF}}</div>
            <li class="aui-border-b" v-for="(item2, index2) in item.item" @click="openRemark(index, index2)">
              ...
              <div class="remark-box clearfix hide" :ref="'remark-' + index + '-' + index2">
...
//          })
    ...
    methods: {
      ...
      openRemark(ref,ref2){
        var remark_box = this.$refs['remark-' + ref + '-' + ref2][0]
