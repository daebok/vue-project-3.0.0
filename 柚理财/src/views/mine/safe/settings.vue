<template>
  <div class="page" id="settings">
    ...
    <div class="margin-lr-15 margin-b-30 margin-t-30">
      ...
      authEvent(){
        if(this.resdata.realnameStatus != 1){
          ...
          if(this.resdata.authorizated == 0){ //post方式去授权
            ...
            this.$router.push('/mine/safe/authSign?from=settings')
          }else{
            this.$messagebox({
              title: ' ',
              ...
              if(action == 'confirm'){
                this.$indicator.open({spinnerType: 'fading-circle'}) //提示初始化加载
                this.$http.get(ajaxUrl.authSignCancel, {params: this.getParams}).then((res) => {
                  this.$indicator.close()
                  if(res.data.resCode == '39321'){
                    this.$messagebox({
                      title: ' ',
...
<style scoped>
  ...
  .logout { text-align:center; line-height:.4rem; margin-top: .2rem; background: url(../../../assets/images/public/button_able.png) no-repeat center;}
</style>
