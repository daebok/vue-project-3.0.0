  ...
  <div class="page payment-page" id="pageDetail">
    ...
    <div v-show="!showRes" class="form-invest">
      ...
      <div v-else class="form-list">
        <label>投资金额(元)</label>
        <input ref="inputMoney" type="tel" pattern="\d*" v-model.trim="investMoney" class="invest-money" id="invest-money" :placeholder="resdata.stepAccount > 0 ? '递增金额为'+resdata.stepAccount+'元': '请输入100的整数倍'" :readonly="times2" @input="inputNum" />
      </div>
      ...
      <p class="tips">
        <span class="allIn" v-show="times2">剩余金额小于等于起投金额的两倍，请全部投资</span>
    ...
    methods: {
      ...
      _init() {
        ...
        this.$http.get(ajaxUrl.initInvest, {params:getParams}).then((res) => {
          ...
          // console.log(this.resdata.mostAccount);
      ...
      investInterest(money, upApr=0){
        ...
        money = money || 0
        if(money > this.resdata.mostAccount){
          money = this.resdata.mostAccount
        }
