  ...
  <div class="page payment-page" id="pageDetail">
    ...
    <div v-show="!showRes" class="form-invest">
      ...
      <div class="form-group margin-b-10">
        <label>投资区间</label>
        <span class="color-333">{{resdata.bondLowestMoney}}元 - {{resdata.bondMostMoney}}元</span>
      </div>
      ...
      <div class="form-list">
        <label>投资金额(元)</label>
        <!-- 全额受让 -->
        <input v-if="sellStyle == 1" ref="inputMoney" type="number" v-model.trim="investMoney" readonly />
        <!-- 部分受让 -->
        <input v-else-if="sellStyle == 0" @keyup="getInvestData()" ref="inputMoney" type="number" v-model.trim="investMoney" placeholder="请输入投资金额"/>
      </div>
      <div class="form-group margin-b-20 earnings">
        ...
        <span v-if="sellStyle == 1" class="main-color">{{ aprMoney | currency('',2) }}元</span>
        <span v-else-if="sellStyle == 0" class="main-color">{{ changeMoney | currency('',2) }}元</span>
      </div>
      <div class="form-group margin-b-20 earnings">
        <label style="width: 20%;">预期收益</label>
        <span v-if="sellStyle == 1" class="main-color">{{ interest | currency('',2) }}元</span>
        <span v-else-if="sellStyle == 0" class="main-color">{{ preInterest | currency('',2) }}元</span>
      </div>
      ...
      <div class="operator">
        ...
        <mt-button v-show="investMoney" size="large" type="danger" @click.prevent="submitForm">确认支付
          <span v-if="sellStyle == 1" class="font-arial">{{ actualInvestMoney | currency('',2) }}</span>
          <span v-else-if="sellStyle == 0" class="font-arial">{{ realPayAmount | currency('',2) }}</span>元
        </mt-button>
  ...
  export default {
    ...
    methods: {
      ...
      submitForm(){
        ...
        if(this.sellStyle == 1){ //全部受让
          this.realPayAmount = this.actualInvestMoney;
        }
