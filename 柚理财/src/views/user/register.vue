      ...
      <div class="form-area">
        <form id="form_reg">
          <ul>
            ...
            <li>
              <rd-field type="tel" v-model="reg.code" :attr="{maxlength: 6}" label="验证码" placeholder="请输入验证码" name="code" specInput="codeInput">
                <button v-if="!reg.mobile" class="yzm color-999 aui-border-l ellipsis" disabled>发送验证码</button>
                <button v-else @click.prevent="getCode" class="yzm aui-border-l ellipsis">发送验证码</button>
              </rd-field>
            </li>
            <li>
          ...
          <div class="btn-con">
            ...
            <p class="agreement-wrapper">点击“注册”按钮，表示您已阅读并同意<a v-for="item in protocolList" @click="readProtocol(item.id,item.name)" class="protocol main-color">《{{item.name}}》</a></p>
          </div>
        </form>
      </div>
    </div>
...
<style type="text/css" scoped>
  ...
  .form-area ul li .yzm { color: #ff9545;}
  .agreement-wrapper{color: #aaa; margin-top: .15rem;}
</style>