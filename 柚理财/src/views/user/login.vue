...
<script type="text/ecmascript-6">
  ...
  import * as ajaxUrl from '../../ajax.config.js'; // 引入登录接口地址
    ...
    methods: {
      ...
      checkForm() {
        if (this.regexPhone(this.mobile) && this.regexPwd(this.loginPwd)) {
          ...
          this.$http.post(ajaxUrl.loginAjax, formData).then((res) => {
            let errorMsg = res.data.resMsg;
            if (errorMsg !== 'SUCCESS') {
              if (res.data.resCode == 20481) {
                this.$messagebox({
                  title: ' ',
      ...
      getVipLevel(sid) {
        this.$http.get(ajaxUrl.vip, {params: { __sid: sid }}).then((res) => {
          sessionStorage.vipLevel = res.data.resData.userVipLevel;
        })
      }
    },
...
<style type="text/css" scoped>
  ...
  .mint-header .mint-button{color: #fff;}
</style>