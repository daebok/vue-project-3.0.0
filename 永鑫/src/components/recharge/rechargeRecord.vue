    ...
    <mt-loadmore :top-method="loadTop" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" ref="loadMore">
      <ul>
        <div v-for="(item1, index1) in dataList">
          ...
          <li v-for="(item2, index2) in item1.item" class="showed" :id="'recharge-li' + (index1 + 1) + '-' + (index2 + 1)" @click="showBottom(index1 + 1, index2 + 1)">
            <div class="top">
              <span class="left">{{ item2.statusStr }}</span>
              <span class="right">+{{ item2.amount | currency('',2) }}</span>
            </div>
            <div class="middle">{{ item2.createTime | dateFormatFun(4) }}</div>
            <div class="bottom" :id="'recharge-bottom' + (index1 + 1) + '-' + (index2 + 1)">
              <div class="more">
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      dataLoad(type) {
        this.$http.get(ajaxUrl.getRechargeList, { params: this.params }).then((res) => {
          this.noData = false;
          if (res.data.resData) {
      ...
      showBottom(ref1, ref2) {
        let target = document.getElementById('recharge-li' + ref1 + '-' + ref2);
        let height = document.getElementById('recharge-bottom' + ref1 + '-' + ref2).children[0].getBoundingClientRect().height;

        if (target.className.indexOf('showed') >= 0) {
          target.className = '';
          document.getElementById('recharge-bottom' + ref1 + '-' + ref2).style.height = height + 'px';
        } else {
          target.className = 'showed';
          document.getElementById('recharge-bottom' + ref1 + '-' + ref2).style.height = 0;
        }
      }
    }
  }
