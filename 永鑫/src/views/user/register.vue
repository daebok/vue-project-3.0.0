      ...
      <div class="form-area">
        <form id="form_reg">
          <ul>
            ...
            <li>
              <rd-field :type="changeType" v-model="reg.pwdVal" label="登录密码" placeholder="8-16位字符，包含数字和字母" @changeEye="hideShowPwd" specInput="pwdInput" switchEye :attr="{maxlength: 16}" name="pwd"></rd-field>
            </li>
            <li>
              <rd-field type="text" name="realName" v-model="reg.realName" label="真实姓名" :attr="{maxlength: 10}" placeholder="请输入真实姓名"></rd-field>
            </li>
            <li>
              <rd-field type="text" name="idNo" v-model="reg.idNo" label="身份证号" :attr="{maxlength: 18}" placeholder="请输入身份证号码"></rd-field>
            </li>
            <!-- <li>
          ...
          <div class="btn-con">
            ...
            <!-- 控制按钮是否可点 -->
            <mt-button v-if="reg.mobile && reg.code && reg.pwdVal && reg.realName && reg.idNo" @click.prevent="checkForm" type="danger" size="large">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 注册表单提交校验
      checkForm(e) {
        if (this.regexPhone(this.reg.mobile) && this.regexPwd(this.reg.pwdVal) && this.chName(this.reg.realName)) {
          if(this.regexIdCard(this.reg.idNo)){
            if (!this.agree) {
            ...
            let submitParams = Object.assign(this.reg, { pwd: pwd, openId: this.openId});
            let self = this;
              ...
              if (res.data.resData !== '') {
                ...
                self.$http.post(ajaxUrl.doRegister, qs.stringify(param)).then((res) => {
                  if (res.data.resData !== '') {
                    ...
                    self.$router.push('/register/'+this.openId+'/regSucc');//注册落地页
                    //self.$router.push('/invest?openId='+this.openId);
                    // this.$messagebox({
                    //   title: ' ',
                    ...
                    //   message: '恭喜您注册成功'
                    // }).then(res => {
                    //   window.location.href = '/invest?openId='+this.openId;//跳转列表页
                    // });
                  } else {
...
<style type="text/css" scoped>
  ...
  .form-area ul li .yzm {width: .9rem;color: #F95A28; outline: none;}
</style>