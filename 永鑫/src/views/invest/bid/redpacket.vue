        ...
        <li v-for="item in list" :class="{'color-999':investMoney < item.useTenderMoney}">
          <div class="coupon-list-1">
            <div class="coupon-left" :class="{'color-999':investMoney < item.useTenderMoney}">
              <i class="symbol">¥</i><i class="coupon-num">{{item.amount}}</i>
            </div>
            ...
            <label class="label-check">
              <input class="checkbox hide" :data-amount="item.amount" :ref="item.uuid" type="checkbox" :value="item.uuid" v-model="selectedList" :disabled="investMoney < item.useTenderMoney">
  ...
  export default {
    ...
    methods: {
      projectList(page = 1){
        ...
        if(this.$route.params.projectId == 'fund') {  // 粮票宝进来的就传type
          Object.assign(urlParams, {type: 'fund'})
        }else{
          Object.assign(urlParams, {projectId: this.$route.params.projectId})
        }
