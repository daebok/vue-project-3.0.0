        ...
        <ul>
          ...
          <li>
            <rd-field type="tel" class="get-code" v-model="code" :attr="{maxlength: 6}" label="验证码" placeholder="请输入验证码" name="code" specInput="codeInput">
              <button ref="sendCode" @click.prevent="getCode" class="yzm main-color aui-border-l ellipsis">获取验证码</button>
            </rd-field>
          </li>
          </ul>
        </form>
...
<script>
  ...
  export default {
    ...
    methods: {
      getCode (evt) {
        // 获取验证码
        this.$http.get(ajaxUrl.modifyEmailCode, {params: this.getParams}).then((res) => {
